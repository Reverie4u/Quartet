...
if __name__ == '__main__':
    ...
    for i in range(end_size, max_size):
        for j in range(count):
            ...
            dataset2_test_target = np.append(dataset2_test_target,
                                             label[i * batch + j * 20 + 14:i * batch + j * 20 + 20], axis=0)
    train = np.append(dataset1_train, dataset2_train, axis=0)
    train_target = np.append(dataset1_train_target, dataset2_train_target, axis=0)
    test = np.append(dataset1_test, dataset2_test, axis=0)
    test_target = np.append(dataset1_test_target, dataset2_test_target, axis=0)
    ...
    for step in range(training_step):
        ...
        dataset2_test_pre = model(dataset2_test)
        dataset2_acc = (np.argmax(dataset2_test_pre.data.cpu().numpy(),
                                  axis=1) == dataset2_test_target).sum() / dataset2_test_len
        ...
        if step + 1 == training_step:
            ...
            print("\t dataset2_acc=%f" % (dataset2_acc))
        px.append(step)

...
if __name__ == '__main__':
    ...
    for step in range(training_step):
        ...
        if step + 1 == training_step:
            writeCsv.writecsv("./data/TBOP_test_pre_job.csv", np.argmax(test_pre.data.cpu().numpy(), axis=1))
    torch.save(model, "TBOP_job_model.pkl")
    # store train_loss and test_loss
    writeCsv.writecsv("./data/TBOP_job_train_loss.csv", np.array(py))
    writeCsv.writecsv("./data/TBOP_job_test_loss.csv", np.array(pz))
    # store train_acc and test_acc
    writeCsv.writecsv("./data/TBOP_job_train_acc.csv", np.array(py1))
    writeCsv.writecsv("./data/TBOP_job_test_acc.csv", np.array(pz1))
    ...
    plt.savefig("./images/train_TBOP_job.png")

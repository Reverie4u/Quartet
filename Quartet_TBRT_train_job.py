...
if __name__ == '__main__':
    ...
    job_train_index = [0, 1, 2, 4, 5, 6, 8, 9, 11, 12, 14, 15, 17, 18, 19, 20, 23, 24, 26, 27, 28, 30, 31, 32, 34,
                       35,
                       37, 38, 39, 41, 42,
                       44, 45, 46, 48, 49, 51, 52, 53, 55, 56, 57, 59, 60, 61, 62, 65, 66, 68, 69, 70, 72, 73, 75,
                       76,
                       78, 79, 80, 82, 83,
                       85, 87, 88, 90, 91, 93, 94, 96, 97, 99, 100, 102, 103, 105, 106, 108, 110, 111, 113, 115,
                       117,
                       119, 121, 123, 125,
                       127, 129, 131, 133, 135]
    ...
    for step in range(training_step):
        ...
        if step + 1 == training_step:
            writeCsv.writecsv("./data/TBRT_test_pre_job.csv", np.argmax(test_pre.data.cpu().numpy(), axis=1))
    torch.save(model, "TBRT_job_model.pkl")
    # store train_loss and test_loss
    writeCsv.writecsv("./data/TBRT_job_train_loss.csv", np.array(py))
    writeCsv.writecsv("./data/TBRT_job_test_loss.csv", np.array(pz))
    # store train_acc and test_acc
    writeCsv.writecsv("./data/TBRT_job_train_acc.csv", np.array(py1))
    writeCsv.writecsv("./data/TBRT_job_test_acc.csv", np.array(pz1))
    ...
    plt.savefig("./images/train_TBRT_job.png")

...
if __name__ == '__main__':
    ...
    for step in range(training_step):
        ...
        pz1.append(test_acc)
    torch.save(model, "TBOH_tpch_model.pkl")
    # store train_loss and test_loss
    writeCsv.writecsv("./data/TBOH_tpch_train_loss.csv", np.array(py))
    writeCsv.writecsv("./data/TBOH_tpch_test_loss.csv", np.array(pz))
    # store train_acc and test_acc
    writeCsv.writecsv("./data/TBOH_tpch_train_acc.csv", np.array(py1))
    writeCsv.writecsv("./data/TBOH_tpch_test_acc.csv", np.array(pz1))
    ...
    plt.savefig("./images/train_TBOH_tpch.png")
